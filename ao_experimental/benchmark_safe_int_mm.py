    ...
    for n, k, m in itertools.product(n_vals, k_vals, m_vals):
        ...
        results.append(
            [
                ...
                safe_int_mm_speedup,
                int_mm_trit_speedup,
                int_mm_trit_t_speedup,
            ]
        )

        print(
            f"  half_eag_ms: {mm_half_ms:0.2f}, int8_eag_ms: {int_mm_ms:0.2f}, int8_safe_eag_ms: \
            {safe_int_mm_ms:0.2f}, int8_trit_ms: {trit_int_mm_ms:0.2f}, int8_trit_t_ms: {trit_int_t_mm_ms:0.2f}"
        )
        print(
            f"  eag_speedup: {int_mm_speedup:0.2f}, safe_eag_speedup: {safe_int_mm_speedup:0.2f}, \
            trit_speedup: {int_mm_trit_speedup:0.2f}, trit_t_speedup: {int_mm_trit_t_speedup:0.2f}"
        )
        print("\n")
