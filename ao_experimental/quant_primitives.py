...
def dynamically_quantize_per_channel(x: torch.Tensor, quant_min: int=-128, quant_max: int=127, target_dtype: torch.dtype = torch.int8, axis: int = 0):
    ...
    def get_min_max_per_channel(x: torch.Tensor, axis: int):
        if len(x.shape)==2:
            return torch.aminmax(x, dim = axis-1)
        elif len(x.shape)>2:
            x_dim = x.shape
    ...
    x_div = x.transpose(0, axis) / scale ## TODO HERE
    x_round = torch.round(x_div)
    x_zp = x_round + zero_point
    x_zp = x_zp.transpose(0, axis) ## TODO HERE
...
def dequantize_per_tensor(int_repr: torch.IntTensor, scale, zero_point, out_dtype=torch.float32):
    return (int_repr.to(out_dtype) - zero_point) * scale

# reference: https://fburl.com/code/org0fmi3
def dequantize_per_channel(int_repr: torch.IntTensor, scales, zero_points, out_dtype=torch.float32, axis: int=0):
    # assumes axis is 0
    y = int_repr.transpose(0, 1) ## TODO HERE
    y = y.to(out_dtype)
    y = y - zero_points
    y = y * scales
    y = y.transpose(0, 1) ## TODO HERE
    return y
...
def quant_int8_matmul(
    ...
    """###### Assumes that axis is 0 ## TODO CDH"""
